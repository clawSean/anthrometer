...
export function paneHasReplPrompt(pane) {
...
async function killSession(sessionName) {
    try {
        await sh(`tmux kill-session -t ${q(sessionName)} 2>/dev/null`, 5000);
    }
    catch { }
}
/**
...
async function ensureSessionReady(sessionName, claudeCommand, maxWaitMs = 18000) {
    ...
    try {
        await sh(`tmux send-keys -t ${q(sessionName)} Escape 2>/dev/null`, 3000);
    }
    catch { }
    ...
    while (Date.now() - start < maxWaitMs) {
        ...
        if (paneHasTrustPrompt(lastPane)) {
            ...
            try {
                await sh(`tmux send-keys -t ${q(sessionName)} Enter`, 3000);
            }
            catch { }
            await delay(3000);
            continue;
...
export default function register(api) {
    ...
    const handler = async (ctx) => {
        try {
            ...
            const parsed = parseUsage(raw, new Date(), { sdkOverage: sdkProbe?.rateLimitInfo || null });
            if (wantRaw)
