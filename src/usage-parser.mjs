...
function fmtWindowBlock(title, section, emoji = "•") {
  ...
  if (section.resetText || section.resetIn) {
    ...
    lines.push(`   _⏰ Reset: ${parts.join(" · ")}_`);
  }
...
export function formatUsage(parsed) {
  ...
  const modeSuffix = parsed?.mode && parsed.mode !== "unknown" ? ` · _${parsed.mode}_` : "";
  ...
  if (parsed?.api) {
    ...
    const apiReset = formatResetLine("• ⏰ *Reset:* ", a.resetText, a.resetIn);
    if (apiReset) lines.push(`_${apiReset}_`);
  }

  const ex = parsed?.extra;
  if (ex) {
    const exStatus = ex.status || "enabled";
    lines.push("", `*💸 Extra Usage*${exStatus ? ` — _${exStatus}_` : ""}`);
    ...
    if (ex.status === "not enabled") {
      ...
      const exReset = formatResetLine("• ⏰ *Reset:* ", ex.resetText, ex.resetIn);
      if (exReset) lines.push(`_${exReset}_`);
    } else {
      lines.push("• Unknown");
    }
  }
